...
@Component({
  ...
  template: `
    <div @fadeAnimation class="fixed inset-0 z-[100] bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center transition-colors duration-300">
      <div class="relative mb-8">
        <div class="absolute inset-0 bg-indigo-500/20 blur-3xl rounded-full animate-pulse"></div>
        <div class="w-24 h-24 bg-gradient-to-br from-indigo-500 to-purple-600 dark:from-indigo-600 dark:to-purple-700 rounded-3xl flex items-center justify-center text-white shadow-2xl shadow-indigo-500/30 dark:shadow-indigo-900/50 relative z-10 animate-[bounce_2s_infinite]">
      ...
      <div class="absolute bottom-12 flex flex-col items-center gap-4">
        <div class="w-8 h-8 border-2 border-indigo-100 dark:border-indigo-900 border-t-indigo-600 dark:border-t-indigo-400 rounded-full animate-spin"></div>
        <p class="text-xs text-slate-400 dark:text-slate-600">Loading v1.0.0...</p>
      </div>
    </div>
  `
})
